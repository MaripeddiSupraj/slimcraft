...
# 2. Let the agent rewrite it and open a PR
$ slimcraft harden ./Dockerfile --pr
...
*   **Deterministic Core:** We use real AST parsing and `docker daemon` integration. The LLM is strictly gated behind `--rewrite`.
*   **Safe by Default:** We validate the rewritten Dockerfile by ensuring it builds cleanly before suggesting it.
*   **Privacy First:** Run purely locally using Ollama (`--model local:qwen2.5-coder`), or use the Anthropic API for maximum reasoning. Proprietary Dockerfiles don't have to leave your laptop.
